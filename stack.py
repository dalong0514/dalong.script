...
for infile in glob.glob("/Users/Daglas/Desktop/*.json"):
    ...
    views = []
    for reviews in sorted(dictls, key = lambda e:e.__getitem__('vote'), reverse=True):
        views.append(reviews['title'])
        views.append(reviews['author'])
        views.append(reviews['url'])
        views.append(reviews['question'])
        views.append(reviews['date'])
        views.append(reviews['vote'])
        views.append(reviews['content'])
